...
def find_threshold(batch_size=512, timesteps=2500, architecture='VGG16'):
    ...
    loader = torch.utils.data.DataLoader(dataset=trainset, batch_size=batch_size, shuffle=True, num_workers=4, generator=torch.Generator(device='cuda') if device.type == 'cuda' else None)
        ...
        for batch_idx, (data, target) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    f.write('\n Error in forward pass for layer {}: {}'.format(layer, str(e)))
                    f.flush()
                    max_act = 1.0
                    break
    ...
    if architecture.lower().startswith('vgg'):              
        for l in obj.features.named_children():
            if isinstance(l[1], nn.Conv2d):
                f.write('\n Processing conv layer: {}'.format(l[0]))
                f.flush()
        ...
        for c in obj.classifier.named_children():
            if isinstance(c[1], nn.Linear):
                if (int(l[0])+int(c[0])+1) == (len(obj.features) + len(obj.classifier) -1):
                    ...
                    f.write('\n Processing linear layer: {}'.format(c[0]))
                    f.flush()
...
if __name__ == '__main__':
    ...
    if dataset in ['CIFAR10', 'CIFAR100']:
        ...
        normalize_gray = transforms.Normalize(mean=[0.5], std=[0.5])  # grayscale용
        transform_train = transforms.Compose([
    ...
    if dataset == 'CIFAR10':
        ...
        labels = len(trainset.classes)
    
    train_loader    = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=4,
     generator=torch.Generator(device='cuda') if device.type == 'cuda' else None)
    test_loader     = DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=4, # 적절한 CPU 코어 수로 설정 (예: 4 또는 os.cpu_count())
     generator=torch.Generator(device='cuda') if device.type == 'cuda' else None)
    ...
    if pretrained_ann:
      ...
      for key in state['state_dict'].keys():
          if key in cur_dict:
              if (state['state_dict'][key].shape == cur_dict[key].shape):
                  ...
                  f.write('\n Error: Size mismatch, size of loaded model {}, size of current model {}'.format(state['state_dict'][key].shape, cur_dict[key].shape))
                  skipped_count += 1
          else:
      ...
      f.write('\n\n Summary: Loaded {} layers, Skipped {} layers'.format(loaded_count, skipped_count))
