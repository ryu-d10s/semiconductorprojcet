...
class VGG(nn.Module):
    def __init__(self, vgg_name='VGG16', labels=10, dataset = 'CIFAR10', kernel_size=3, dropout=0.2):
        ...
        if vgg_name == 'VGG5' and dataset == 'MNIST':
            ...
            if vgg_name == 'VGG5':
                self.classifier = nn.Sequential(
                    nn.Linear(128*16*16, 4096, bias=False),  # 64x64 image: 2 poolings -> 16x16
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.5),
                    nn.Linear(4096, 4096, bias=False),
                    nn.ReLU(inplace=True),
                ...
                self.classifier = nn.Sequential(
                    nn.Linear(512*4*4, 4096, bias=False),  # 64x64 image: 4 poolings -> 4x4
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.5),
                    nn.Linear(4096, 4096, bias=False),
                    nn.ReLU(inplace=True),
    ...
    def _make_layers(self, cfg):
        ...
        if self.dataset in ['MNIST', 'HAND_GESTURE']:
            in_channels = 1
        else:
            in_channels = 3
