...
def train(epoch):
    ...
    pbar = tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Train Epoch {epoch}", unit="batch")
    ...
    for batch_idx, (data, target) in pbar:
        ...
        losses.update(loss.detach(), data.size(0))
        top1.update(correct.float()/data.size(0), data.size(0))
...
def test(epoch):
    ...
    with torch.no_grad():
        model.eval()
        global max_accuracy
        pbar = tqdm(enumerate(test_loader), total=len(test_loader), desc=f"Test Epoch {epoch}", unit="batch")
        for batch_idx, (data, target) in pbar:
...
if __name__ == '__main__':
    ...
    test_loader     = DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=8, # 적절한 CPU 코어 수로 설정 (예: 4 또는 os.cpu_count())
     generator=torch.Generator(device='cuda') if device.type == 'cuda' else None)
