...
class VGG_SNN_STDB(nn.Module):
	...
	def _make_layers(self, cfg):
		...
		elif self.dataset == 'HAND_GESTURE':
			if self.vgg_name == 'VGG5':
				layers += [nn.Linear(128*16*16, 4096, bias=False)]
			else:
	...
	def neuron_init(self, x):
		...
		for l in range(len(self.features)):
			...
			if isinstance(self.features[l], nn.Conv2d):
				self.mem[l] 		= torch.zeros(self.batch_size, self.features[l].out_channels, self.width, self.height, device=device)
			
			elif isinstance(self.features[l], nn.Dropout):
				self.mask[l] = self.features[l](torch.ones(self.mem[l-2].shape, device=device))
		...
		for l in range(len(self.classifier)):
			...
			if isinstance(self.classifier[l], nn.Linear):
				self.mem[prev+l] 		= torch.zeros(self.batch_size, self.classifier[l].out_features, device=device)
			
			elif isinstance(self.classifier[l], nn.Dropout):
				self.mask[prev+l] = self.classifier[l](torch.ones(self.mem[prev+l-2].shape, device=device))
